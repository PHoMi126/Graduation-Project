using System.Collections.Generic;
using UnityEditor;
using UnityEngine;

namespace Smart
{
    public partial class LevelEditor
    {
        public class Pick
        {
            //
            // Variables
            //

            private float distance;
            private float height;
            private Vector3 mouse;
            private Plane plane;
            private Bounds bounds;
            private Transform[] transforms;
            private List<GameObject> gameObjects;

            private Ray ray;
            private RaycastHit rayInfo;

            public Event e;
            public Camera camera;

            //
            // Properties
            //

            public bool Hit
            { get; protected set; }

            public GameObject Target
            { get; protected set; }

            public Vector3 Position
            { get; protected set; }

            public Vector3 Point
            { get; protected set; }

            public Vector3 TargetPosition
            {
                get { return Target.transform.position; }
            }

            //
            // Methods
            //

            public void Set(float height, Event e, Camera camera)
            {
                this.height = height;
                this.e = e;
                this.camera = camera;
            }

            public void Update()
            {
                UpdateRay();
                UpdatePosition();
                UpdateCast();
            }


            public void UpdateRay()
            {
                mouse = e.mousePosition;

                if (mouseDownAndShift)
                {
                    Vector2 abs = new(Mathf.Abs(e.delta.x), Mathf.Abs(e.delta.y));
                    // Lock y
                    if(abs.x > abs.y)
                    {
                        mouse.y = mouseShifted.y;
                    }
                    // Lock x
                    else
                    {
                        mouse.x = mouseShifted.x;
                    }
                }

                mouse.y = camera.pixelHeight - mouse.y;

                ray = camera.ScreenPointToRay(mouse);
            }

            public void UpdatePosition()
            {
                if (view2D)
                {
                    //2D
                    plane = new Plane(Vector3.back, new Vector3(0, 0, height));
                }
                else
                {
                    //3D
                    plane = new Plane(Vector3.up, new Vector3(0, height, 0));
                }

                plane.Raycast(ray, out distance);

                Position = ray.GetPoint(distance);
            }

            public void UpdateCast()
            {
                Hit = Physics.Raycast(ray, out rayInfo);

                Point = rayInfo.point;

                Target = rayInfo.collider ? rayInfo.collider.gameObject : null;
            }

            public void GetGameObject(LayerMask layer)
            {
                Target = HandleUtility.PickGameObject(e.mousePosition, true);

                if(null == Target)
                {
                    return;
                }

                if (!WorkingLayers(Target, layer))
                {
                    Target = null;
                }
            }

            public bool Overlap(Vector3 center, Vector3 size, GameObject[] previews, LayerMask layer)
            {
                transforms = FindObjectsOfType<Transform>();

                RemovePreviews(previews);

                bounds = new Bounds(center, size * 0.99f);
                Renderer otherRnd;
                Bounds otherBounds;
                Renderer previewRnd;
                Bounds previewBounds;

                for (int i = 0; i < transforms.Length; i++)
                {
                    if (null == transforms[i])
                    {
                        continue;
                    }

                    if (!WorkingLayers(transforms[i].gameObject, layer))
                    {
                        continue;
                    }

                    otherRnd = transforms[i].GetComponentInChildren<Renderer>();

                    if (null == otherRnd) { continue; }

                    otherBounds = otherRnd.bounds;
                    otherBounds.size *= 0.99f;

                    // Check agains each of the possible renderers that we have
                    for (int j = 0; j < previews.Length; j++)
                    {
                        previewRnd = previews[j].GetComponentInChildren<Renderer>();

                        if (null == previewRnd) { continue; }

                        previewBounds = previewRnd.bounds;

                        if (previewBounds.Intersects(otherBounds))
                        {
                            return true;
                        }
                    }

                    //if (bounds.Contains(transforms[i].position)) { return true; }
                }

                return false;
            }

            void RemovePreviews(GameObject[] previews)
            {
                Transform[] pt;

                for (int i = 0; i < transforms.Length; i++)
                {
                    for (int j = 0; j < previews.Length; j++)
                    {
                        pt = previews[j].GetComponentsInChildren<Transform>();

                        for (int w = 0; w < pt.Length; w++)
                        {
                            if (transforms[i] == pt[w])
                            {
                                transforms[i] = null;
                            }
                        }
                    }
                }
            }

            // No need to remove previews here, since this func is used with delete mode
            // at that point we are not rendering previews
            public GameObject[] GetGameObjects(Vector3 center, Vector3 size, LayerMask layer)
            {
                gameObjects = new List<GameObject>();

                transforms = FindObjectsOfType<Transform>();

                bounds = new Bounds(center, size * 0.99f);

                for (int i = 0; i < transforms.Length; i++)
                {
                    if (!WorkingLayers(transforms[i].gameObject, layer))
                    {
                        continue;
                    }

                    if (bounds.Contains(transforms[i].position))
                    {
                        gameObjects.Add(transforms[i].gameObject);
                    }
                }

                return gameObjects.ToArray();
            }

            bool WorkingLayers(GameObject gameObject, LayerMask layer)
            {
                return layer == (layer | (1 << gameObject.layer));
            }
        }
    }
}