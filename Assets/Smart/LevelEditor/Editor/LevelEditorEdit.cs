    ...
    public partial class LevelEditor
    {
        ...
        void EditModeRect()
        {
            ...
            for (int i = 0; i < transforms.Length; i++)
            {
                ...
                sprRnd = transforms[i].GetComponentInChildren<SpriteRenderer>();

                if (sprRnd && transforms[i].hasChanged)
                {
                    Undo.RecordObject(transforms[i], "sle.move");
                ...
                if(sprRnd)
                {
                    Undo.RecordObject(sprRnd, "sle.rect");

                    sprRnd.size = Snap(sprRnd.size, scale);
                }

                if (sprRnd)
                {
                    SceneLabel(transforms[i].position + labelObjectOffset, sprRnd.size.ToString());
                }

                SceneLabel(transforms[i].position + labelObjectOffset * -1, transforms[i].position.ToString());
            }
